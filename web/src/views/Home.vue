<template>
  <div class="page-container home-page">
    <n-grid :cols="3" :x-gap="16" :y-gap="16">
      <n-gi>
        <n-card class="module-card management" hoverable @click="router.push('/management/projects')">
          <div class="module-icon">PM</div>
          <h3>项目管理</h3>
          <p class="module-desc">项目树、任务看板、文档</p>
          <div class="module-stats">
            <n-statistic label="项目数量" :value="stats.projectCount" />
          </div>
        </n-card>
      </n-gi>
      <n-gi>
...
const stats = ref({ projectCount: 0, paperCount: 0, modelCount: 0 })
...
onMounted(async () => {
  ...
  try {
    ...
    stats.value.projectCount = projs?.length || 0
  } catch {}
  try {
