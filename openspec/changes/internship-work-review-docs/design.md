...
**Goals:**
- 7 篇平铺文档：道层（数字人要点）+ 工作深潜（身份一致性 / 工程改进含 PasteBack / 实时性改进）+ 综合（行业全景 / 发文方向 / 总览），每篇讲清：问题是什么 → 怎么定位 → 做了什么 → 效果如何 → 哪些没成
- 支撑工作汇报：量化成果总表可直取直用
...
|---|------|------------------------------|------|-------------|
| 1 | `数字人要点.md` | 道层导论：数字人技术要素框架——动作（音唇同步）、身份（一致性+身份资产）、渲染合成、实时性；每要点链对应术篇；不写工作过程细节 | 技术路线篇（软链） | `docs-intern-fundamentals` |
| 2 | `身份一致性.md` | 只写身份主题：漂移诊断与治理（模长/夹角实验）+ **身份资产与参考提供**（单图/多图/视频/anchor 等方式研究与取舍）；不写音频微调 | — | `docs-intern-identity` |
...
### 1. `数字人要点.md`（结构契约，道层导论）

- `## 要素框架`：数字人系统分解——动作、身份、渲染合成、实时性；各要素核心问题与对应术篇导览
- `## 动作与音唇同步`：驱动信号→动作表示→生成链路要点；音唇同步评价口径（sync_c/sync_d）与常见问题
- `## 身份与身份资产`：身份一致性问题概述（链《身份一致性》）+ 身份参考如何提供（单图/多图/视频/anchor 的取舍）
- `## 渲染与合成`：人脸区域→全帧的合成链路，贴回作为术级技术（链《工程改进》PasteBack 节）
- `## 实时性`：延迟预算分解、TTFF/FFTT/cadence 口径（链《实时性改进》）

### 2. `身份一致性.md`（结构契约，含身份资产）
...
- `## PasteBack 贴回技术`：两条路径（AF 裁剪框+alpha 渐变 vs Ditto 逐帧 M_c2o 仿射，引 paste-back-compositing.md 含历史勘误）、接缝/波动修复（t11-3 active、commit d32fb74）、t24 相关否证——定位：改进合成效果的术级技术
