...
- 子文件夹两个：`management/docs/实习复盘/`（复盘 8 篇）与 `management/docs/论文笔记/`（统一笔记库，合并原"模型介绍"）：**总纲《技术路线》**（视频基座模型 / 动作空间扩散 / 3D GS 三路线 + 代表模型 + 优缺点）+ **五篇深读笔记**（AvatarForcing、Ditto、LiveAct、OmniMate、Talker-T2AV，一篇一个对象，含"在我们体系中的角色"工程层）+ 后续新读论文笔记；夹内平铺，各由独立单篇 change（`docs-note-<slug>`）实施，先整理后动笔
- 复盘 7 篇文档（各自单独 change 实施）：
  ...
  3. `动作.md`（`docs-intern-motion`）— 算法上的改进：分层框架（音唇同步→表情×语言交互→手部/全身），含我们的实验与市面工作总结
