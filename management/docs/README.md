...
2. **审核闸门**（依据 doc-writing skill）：
   ...
   - **用户审大纲** → 通过后进入「资料整理」
   - 素材（`整理-<标题>.md`）**留在 change 目录，不进 docs**
   - **用户审素材** → 通过后才写正文
3. **位置规则**：文章正文进 `实习复盘/`；计划与素材留在 `openspec/changes/<change>/`；外部复制件进 `knowledge/`
4. **事实分级**：结论标注【已验证】【待验证】【已否证】【待补】，不编造数字
5. **引用锚定**：CyberVerse 相关引用标 `@4968280`；知识库引用用相对链接
