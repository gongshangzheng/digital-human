...
**问题**：显式 landmark 稀疏、显式 3DMM 受参数空间限制，能否学到更隐式、更可控的运动表示？这一支常被混为一谈，其实是**两条不同的线**，也是本项目最关心的表示层——它们决定运动能不能被复用、被控制、被低成本传输。

**隐式 3D 关键点线**（FOMM → Face vid2vid → LivePortrait）：运动被表示为学到的 3D 关键点及其形变，身份由 canonical 关键点跨帧复用。

| 工作 | 机构 · 时间 | 表示 | 关键贡献 | 继承自 |
|---|---|---|---|---|
| First Order Motion Model | — | 2D 关键点 + Jacobian | 起点：稀疏关键点 + 局部仿射 warp | — |
| Face vid2vid | NVIDIA · CVPR 2021 Oral | 可分解 3D 隐式关键点 | 三项分解（身份/头姿/表情）、不估 Jacobian、局部自由视角 | FOMM |
| SadTalker | 西安交大 / 腾讯 AI Lab · CVPR 2023 | 3DMM 系数 + 隐式关键点渲染器 | ExpNet 管嘴 + PoseVAE 管头姿 | Wav2Lip、Face vid2vid |
| LivePortrait | 快手 · arXiv 2024 | 隐式关键点 + 显式 scale factor | 单一 ConvNeXt 联合预测、stitching/眼唇重定向、约 6900 万帧 | Face vid2vid |
| X-Portrait | ByteDance · SIGGRAPH 2024 | 无 landmark，RGB 驱动 + ControlNet | 局部 ControlNet 保微表情，Face vid2vid 造跨身份对 | Face vid2vid、ControlNet |
| ChatAnyone | arXiv 2025 | 低维运动空间 + warping GAN | 层级 audio-to-motion，扩到上半身与手势 | LivePortrait |
| PerformRecast | 虎鲸文娱 · CVPR 2026 | 修正的关键点变换顺序 | 改为 (x_c+δ)·R 对齐 FLAME，真正解耦表情与头姿 | LivePortrait、FLAME |
| JoyVASA | 京东健康 & 浙大 · arXiv 2024 | 解耦表征 + 轻量 DiT 运动 | 身份无关运动生成，跨物种驱动 | LivePortrait |
| Ditto | 蚂蚁集团 · ACM MM 2025 | LivePortrait 265D 运动空间 | 把扩散目标搬进运动空间 | VASA-1、LivePortrait |

**潜空间导航线**（LIA → LIA-X）：不用关键点，而是把运动表示为潜空间线性导航 $z_{s\to d}=z_{s\to r}+w_{r\to d}$，再用运动字典把位移分解成因子。

| 工作 | 机构 · 时间 | 表示 | 关键贡献 | 继承自 |
|---|---|---|---|---|
| LIA | — | 潜空间线性导航 + 稠密正交运动字典 | 起点：无关键点的线性运动表示 | — |
| LIA-X | Shanghai AI Lab / Inria · ICCV 2025 | 稀疏运动字典 | 稀疏约束 → 可解释、可编辑；edit-warp-render | LIA |
| FLOAT | KAIST + DeepBrain AI · ICCV 2025 | LIA 系身份-运动隐空间 | 正交基 + OT 流匹配；系数可线性编辑（λ-control） | LIA、VASA-1 |
| Talker-T2AV | HKUST 等 · arXiv 2026 | LIA-X 40 维 motion latent | 共享因果 LM 规划 + 模态专属轻量 DiT 头 | LIA-X |

两条线都属「表示 + warp + render」、都不是 diffusion、也都把身份留在源图侧，但**表示形态不同，且没有直接继承关系**——LIA-X 的前身是 LIA，不是 Face vid2vid。两条线在「运动空间扩散」处汇流：Ditto 用关键点线的 265D 空间做扩散目标，FLOAT / Talker-T2AV 用导航线的运动隐空间做流匹配。**边界**：隐式关键点的几何含义靠探针显影、可解释性弱；潜空间导航的可解释性依赖稀疏约束，而 LIA-X 的稀疏/解耦只有定性证据。
...
这是我们项目的主线与最重要的研究方向，单独把「继承了什么、改了什么」列成表。要点是：它其实是**两条并行的链**，不是一条。

| 前身 → 后继 | 继承了什么 | 改/加了什么 |
|---|---|---|
| FOMM → Face vid2vid | 关键点 + 组合掩码 warp 架构 | 2D 关键点 → 3D 隐式关键点；身份/头姿/表情三项分解；不估 Jacobian；自由视角 |
| Face vid2vid → LivePortrait | canonical 关键点与解耦运动 | 可缩放运动变换、stitching 与眼唇重定向、单一 ConvNeXt、数据扩到约 6900 万帧 |
| Face vid2vid → SadTalker | 隐式关键点渲染器 | 加 ExpNet（嘴）+ PoseVAE（头姿）做音频驱动 |
| Face vid2vid → X-Portrait | 运动迁移与跨身份配对 | 去掉 landmark，改 RGB + 局部 ControlNet |
| LivePortrait → ChatAnyone | 面部运动空间 | 扩到上半身与手势 |
| LivePortrait → PerformRecast | 关键点变换 | 改运算顺序为 (x_c+δ)·R，对齐 FLAME 的解耦 |
| LivePortrait → JoyVASA | 解耦表征 | 运动生成换轻量 DiT，支持动物/卡通 |
| LivePortrait → Ditto | 265D 运动空间 | 把扩散目标搬进运动空间 |
| LIA → LIA-X | 潜空间线性导航 + 运动字典 | 加稀疏约束 → 可解释、edit-warp-render |
| LIA → FLOAT | 身份-运动分解的运动隐空间 | 正交基 + OT 流匹配 + 线性编辑（λ-control） |
| LIA-X → Talker-T2AV | 40 维 motion latent | 共享因果 LM 规划 + 模态专属轻量 DiT 头 |

两条链在「运动空间扩散」处汇流（Ditto 用关键点空间，FLOAT / Talker-T2AV 用导航隐空间）。这是我们判断「表示层是本方向最值得投入之处」的主要依据：关键点线提供了成熟、可复用、可实时渲染的运动空间（LivePortrait 的 265D），导航线提供了可解释、可线性编辑的因子化接口（LIA-X 的稀疏字典）。
