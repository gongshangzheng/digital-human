...
summary: 以 106 篇文章（博客数字人语料 99 篇 + 库内论文笔记）为证据，逐篇抽出「提出的问题 → 给出的解法」，按 13 条技术分线归纳，并专门呈现工作之间的继承/派生关系、2020—2026 演进与机构分布，最后补充关键问题
order: 15
---
...
| 本文档 | 每篇的问题-解法对、技术分线、继承谱系、机构与时间 | 以 106 条结构化记录为唯一数据源 |
...
| 来源 | 数量 | 说明 |
|---|---|---|
| 博客 `AI/数字人` 已发布 | 99 | 论文精读 67 + 系列综述 19 + 工程解读 10 + 评测/产业 3 |
| 库内 `论文笔记/` 独有论文 | 7 | Face vid2vid、FLOAT、FPSAttention、Causal Forcing、OmniMate、Talker-T2AV、Vorch-Streamer 等博客未覆盖者 |

每条记录的结构单元是一个**问题-解法对**：一篇文章可以提出多个问题，因此 `qa` 是一个数组，每个元素含 `question` 与 `answer`。全部 106 条共抽出 **157 组问题-解法对**。
...
**未获取项统计**：机构缺失 36 条（多为系列综述与工程解读，本就无单一机构）、venue 缺失 30 条、年份缺失 32 条、论文作者缺失 32 条（其中多数为综述/工程类无作者，少数因无网络未能核对）。这些条目在总表中标为「未标注」或留空，不做臆造。

**覆盖范围**：博客 99 篇 + 库内独有论文 7 篇 = 106 条；共 157 组问题-解法对。
