...
> 前置阅读：[[knowledge/数字人基础|数字人基础]] 3.2 节。本文回答三个问题：动作空间有哪些表示？Ditto 是怎么把眼部和嘴部拆开的？这些表示各自从哪来？
...
同一机制把 `_lip` 换成 `_eye` 就能单独处理眼部。我们团队的 mouth-isolation 改动正是利用它：把唇通道的基底换成源首帧，避免源视频原有的嘴动泄露进来（详见 [[knowledge/Ditto 改动实践|Ditto 改动实践]]）。
...
- **Avatar Forcing 微调**：FLOAT latent 的 identity-motion 分解（z = z_S + m_S）被我们用来做漂移治理的锚点——见 [[knowledge/Avatar Forcing 微调实践|Avatar Forcing 微调实践]]
