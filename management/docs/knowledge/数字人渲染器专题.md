...
> 前置阅读：[数字人基础](数字人基础.md)、[动作空间专题](动作空间专题.md)。本文讨论“动作已经生成后，谁负责把它画成视频”。项目全景见 [[project:digital-human]]。
...
视频源模式下，逐帧 `f_s[t]`、`x_s[t]`、`kp_s[t]` 会带来源视频自己的嘴动，因此我们才有 [Ditto 改动实践](Ditto_改动实践.md) 中的泄露问题和闭嘴化源方案。
...
在 [Talker-T2AV 模型精读](Talker-T2AV_模型精读.md) 中，LIA-X 是 40D motion → 视频帧的解码侧。当前 A10 profile 下，512²、bf16、batch=1 的 LIA-X decoder 约 333ms/帧，说明渲染器本身可能成为 T2AV 速度瓶颈。
...
细节见 [Avatar Forcing Motion Latent AutoEncoder](Avatar_Forcing_Motion_Latent_AutoEncoder.md)。
...
当前 Ditto 视频源闭嘴化中，LatentSync 是有证据的方案：先把源视频嘴动中性化，再让 Ditto 按目标音频生成嘴形。它解决的是源嘴动泄露，不是替代 Ditto renderer 的全部功能。详见 [音画同步专题](音画同步专题.md)。
