...
- **免标注表达力**：把「丢掉用户条件」的生成结果当作差样本做 DPO，不需要人工偏好标注就把动作推得更有反应。
...
- **preferred** `m^w`：真实视频里提取的运动隐变量（表达力强、回应合适）
- **less-preferred** `m^l`：单独训练的 talking avatar 模型（FLOAT）**只看 avatar 音频**生成的结果

这个设计只推高「主动倾听与反应性」，不改变唇同步与语音驱动运动这些本来已经学好的部分。
