...
这套「拿自己生成的结果当下一段历史」的复用，同时也是长时身份漂移的入口——见 [长时身份漂移](#长时身份漂移历史递归方向游走与静音放大)。
...
论文把 exposure bias 留作未来工作，我们把这一条做成了可检验的诊断：先看误差为什么会累积，再看它实际漂在哪个几何量上，最后看什么条件下最容易放大。

**机制：生成结果会成为下一段的历史。** 块滚动推理里，第 $i+1$ 块把上一块末尾 $l$ 帧当作 offset 拼进输入，并用 rolling KV cache 复用已算出的中间结果——两者装的都是**生成出来的**运动隐变量，不是真实视频里的帧。于是每次预测的小偏差不会在下一段被真实数据纠正，反而被当成新的「事实」继续往前推：

```mermaid
flowchart LR
  B1["第 i 块生成 m_S"] --> OFF["offset：末 l 帧拼入下一块"]
  B1 --> KV["rolling KV cache：复用历史中间结果"]
  OFF --> B2["第 i+1 块生成"]
  KV --> B2
  B2 -->|"生成值成为新历史"| OFF
  B2 -->|"生成值成为新历史"| KV
...
身份项 $z_S$ 全程固定，但它固定的只是 identity latent。$z = z_S + m_S$ 是近似解耦：当运动项的走向一路偏移、把解码点带出训练时见过的高密度区域后，固定 $z_S$ 并不能保证解码出的画面还是同一个人。原生模型在推理期也没有「回看参考帧」的闭环——既没有锚帧库，也没有参考条件注入，方向走偏之后无人纠偏。

**诊断：漂的是方向，不是模长。** 把每一帧的运动隐变量看成一个向量，它有两个可以分别测量的属性：模长 $\lVert r_d \rVert_2$ 表示这一刻的运动量有多大，方向表示它在隐空间里朝哪走。这里 `r_d` 就是方法节里逐帧运动项 $m^i$ 的工程记法，指同一件事。方向用与首帧的夹角度量：

$$
\theta_t = \arccos\!\left(\frac{r_t \cdot r_0}{\lVert r_t \rVert_2 \, \lVert r_0 \rVert_2}\right)
$$

$r_t$ 是第 $t$ 帧的运动隐变量，$r_0$ 是首帧，$\theta_t$ 是两者夹角。两个属性对应两个互斥的假说：若漂移来自「运动量被撑开、挤压身份项」，模长应持续增大；若来自「方向游走」，夹角应持续增大。在 c1 单身份、300 秒长轨迹上同时测两者：

| 假说 | 观测对象 | c1 / 300 秒结果 | 当前裁决 |
|---|---|---|---|
| 范数撑开 | $\lVert r_d \rVert_2$ 是否持续增大 | noguide 2.03 → 2.04 → 2.01（-0.01/min）；GT 与 A1 近乎水平 | **不支持** |
| 方向游走 | 与首帧的夹角、到 GT 方向字典的最近邻 | noguide 10.1° → 15.1° → 17.5°（**+2.1°/min**），最近邻峰值 24.8° | **单身份支持** |

GT 自身的方向游走带宽约 3~5°，noguide 升到 17.5° 以上并持续增长；对照之下，推理期锚点引导（A1）把夹角钉在 3.4° → 3.3° → 2.5°（峰值 6.2°，斜率≈0）。所以问题不是「动得越来越大」，而是**隐空间指向一路偏移**，治理应优先约束方向。另外，noguide 的模长恒定偏低约 14%，可能与画面变模糊有关，其机制尚未验证，不能据此说模长与漂移无关。

**静音时缺少强条件，更容易漂。** 说话时 avatar audio 逐帧约束口型与节奏，这是近乎一一对应的强条件，能把生成轨迹不断拉回正确区域。静音时这层约束基本消失，而倾听状态的表情、微点头、视线本来就有多解；此时模型更多依赖自己上一段生成出来的历史继续往前推，同样的每段小偏差更容易被当成事实累积下来。**这一点是我们的工程观察，并配以与模型条件结构一致的机制解释**——目前还没有固定身份、参考图、随机性与总时长的静音/非静音配对对照，因此不能写成定量因果结论。

**据此做的治理。** 既然证据指向方向，推理期就用锚帧库（该身份的特征字典）引导生成方向：c1 单例 300 秒中 CSIM 从 0.645 提到 0.935，后半段塌方消失，唇同步未观察到下降；不需要重训，可随时开关。这是单身份实验中的有效引导设置，跨身份、跨素材的稳健性尚未验证，因此不是通用部署默认值。其余治理尝试的裁决见下一小节与 [[knowledge/Avatar Forcing 微调实践|Avatar Forcing 微调实践]]。

### 我们的实测：治理与适配

这条线是论文留的「未来工作」，我们把它做成了实验。漂移的机制与诊断已写在上一小节 [长时身份漂移](#长时身份漂移历史递归方向游走与静音放大)，本节只记治理与适配的裁决。
...
3. **指标高不等于可用**：我们的「学习式锚力」身份指标更好却出现头抖，说明身份相似度是标量、漂移是多维的——评估口径的裂缝比模型缺陷更值得警惕（多维漂移的实测见 [长时身份漂移](#长时身份漂移历史递归方向游走与静音放大)）。
...
| $r_d$ | 逐帧运动隐变量向量的工程记法（即方法节逐帧运动项 $m^i$）；模长 = 运动量，方向 = 隐空间中的走向 |
| $\theta_t$ | 第 $t$ 帧运动隐变量与首帧方向的夹角 |
