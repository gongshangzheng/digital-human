# AGENTS.md — Digital Human 数字人研发管理平台
...
```
浏览器 → Vite (3212) → FastAPI (8812) → management/ 目录（Markdown 文件）
...
# 1. 后端
cd digital-human
nohup python3 -m uvicorn server.main:app --host 0.0.0.0 --port 8812 </dev/null > /tmp/backend.log 2>&1 & disown

# 2. 前端
cd digital-human/web
nohup npx vite --port 3212 --strict-port </dev/null > /tmp/frontend.log 2>&1 & disown
```

> ⚠️ **从本项目初始化新项目时必须替换端口**
>
> 前端 3212 / 后端 8812 是本仓库（digital-human）的固定端口，生态分配：ProjFlow=8809/3210、pet-action-recognition=8788/3000、本仓库=8812/3212。若以本仓库为脚手架克隆出新项目，
> 必须全局替换为一组新端口，否则两个项目无法同时运行（启动脚本会检测到端口冲突直接退出）。
>
> 需要同步修改的位置共 3 处：
> 1. `start_services.sh` — 后端 8812、前端 3212（含端口冲突检查逻辑）
> 2. `web/vite.config.js` — `server.port`（3212）和 `/api` 代理 target（8812）
> 3. `server/config.py` — `CORS_ORIGINS` 中的 3212
>
...
│   └── requirements.txt
├── web/                     # Vue 3 前端
│   ├── vite.config.js       # Vite 配置（端口 3212，代理 /api → 8812）
│   ├── src/
...
management/ 目录（Markdown 文件）
  ↓ FastAPI (port 8812, server/routers/management.py → server/parsers/)
前端 Vue 组件 (port 3212)
```
...
data/papers.db（SQLite 数据库）
  ↓ FastAPI (port 8812, server/routers/papers.py → server/db.py)
前端 Vue 组件 (port 3212)
```
...
evaluation/ 目录（JSON 文件）
  ↓ FastAPI (port 8812, server/routers/evaluation.py)
前端 Vue 组件 (port 3212)
```
...
### Vite 端口被占用
使用 `--strict-port` 避免自动 fallback 到其他端口。如果 3212 被占，先 `lsof -i :3212` 找到并杀掉旧进程。

### 管理页面数据为空
检查后端（port 8812）是否在运行，`management/` 目录下是否有对应的数据文件。
