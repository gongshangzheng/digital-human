...
本文档是 `documentation` skill 的参考文件，供在 Digital Human 的 Wiki、论文笔记与项目文档中绘制 Mermaid 图表时查阅。
...
节点标签里可直接写 LaTeX 公式（渲染为 KaTeX，样式由本仓库加载的 `katex.min.css` 提供）：

```mermaid
flowchart LR
  A["输入 $$x_t$$"] --> B["损失 $$\mathcal{L}_{rec}$$"]
```

- 必须用 `$$...$$`；单个 `$` **不渲染**（Mermaid 自身规则）
- 图内公式不需要额外配置，`mermaid.initialize` 已支持
